      ...
      const results = await Promise.all(
        bitmaps.map(
          (bitmap) =>
            new Promise((resolve) => {
              if (!bitmap.image)
                resolve({ type: "CANVAS", bitmap, image: null });
              const img = new Image();
              img.src = bitmap.image.src;
              img.onload = () => resolve({ type: "IMAGE", bitmap, img });
            })
        )
      );
      this._images = results.map(({ type, bitmap, img }) => {
        if (type === "CANVAS") return bitmap.canvas;
        ...
        URL.revokeObjectURL(bitmap._image.src);
        bitmap._revokeStatus = "DONE";
